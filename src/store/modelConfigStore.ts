...
export const useModelConfigStore = defineStore('modelConfig', {
  ...
  actions: {
    updateLightConfig(config: Partial<Pick<ModelConfigState, 'lightIntensity' | 'lightColor' | 'lightPosition'>>) {
    ...
    updateModelConfig(config: Partial<Pick<ModelConfigState, 'modelScale' | 'modelAutoRotate' | 'modelFloatAnimation'>>) {
    ...
    loadFromLocalStorage() {
      ...
      if (savedConfig) {
        try {
          ...
          // 确保所有数值类型正确
          if (config.lightIntensity !== undefined) config.lightIntensity = Number(config.lightIntensity);
          if (config.lightPosition) {
            if (config.lightPosition.x !== undefined) config.lightPosition.x = Number(config.lightPosition.x);
            if (config.lightPosition.y !== undefined) config.lightPosition.y = Number(config.lightPosition.y);
            if (config.lightPosition.z !== undefined) config.lightPosition.z = Number(config.lightPosition.z);
          }
          if (config.modelScale !== undefined) config.modelScale = Number(config.modelScale);
          if (config.modelAutoRotate !== undefined) config.modelAutoRotate = Boolean(config.modelAutoRotate);
          if (config.modelFloatAnimation !== undefined) config.modelFloatAnimation = Boolean(config.modelFloatAnimation);
          if (config.cameraDistance !== undefined) config.cameraDistance = Number(config.cameraDistance);
          if (config.cameraFov !== undefined) config.cameraFov = Number(config.cameraFov);
          if (config.rotationSpeed !== undefined) config.rotationSpeed = Number(config.rotationSpeed);
